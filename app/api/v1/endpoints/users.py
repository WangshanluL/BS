...
from app.schemas.loginAndRegister import LoginRequest, RegisterRequest
...
@router.post("/login")
async def login(request: LoginRequest, db: Session = Depends(get_db_session), client_req: Request = None):
    email = request.email
    ...
    if captcha_input.lower() == captcha_store.get(captcha_input.lower()):
        ...
        if user:
            ...
            # 更新用户登录信息
            user_service.update_login_info(db, user_id=user.user_id, ip=client_ip, ip_address=ip_address)
...
async def register(request: RegisterRequest, db: Session = Depends(get_db_session)):
    ...
    # 使用user_service创建用户
    user = user_service.create_user(db, user_create=user_create)

    # 注意：这里缺少添加欢迎消息的功能，你需要在别的地方实现message_service
    # 或者在user_service中添加add_welcome_message方法
